...
var cubePaddedDilatedObj = [];
var playErosion = false;
var playDilation = true;
...
var updateKernelPosition = function(){
    if(playErosion==true && playAnimationVar==true){
        console.log(paddedDilateArray);
        if(imageType=='Binary'){
            var isUpdatedWithZero = false;
            colorTheCubeToDefault(paddedDilateArray, cubePaddedDilatedObj);
            for(y=paddedDilateArray.length-1;y>=paddedDilateArray.length-1-mk+1;y--){
                for(x=0;x<nk;x++){
                    if(kernelArray[paddedDilateArray.length-1-y][x]=='1'){
                        if(paddedDilateArray[paddedDilateArray.length-1-y+updatedPositiony][updatedPositionx+x]=='0'){
                            cubePaddedDilatedObj[paddedDilateArray.length-1-y+updatedPositiony][updatedPositionx+x].material.color.setHex( 0xfbdd11 );
                            isUpdatedWithZero = true;
                        }
                        else{
                            cubePaddedDilatedObj[paddedDilateArray.length-1-y+updatedPositiony][updatedPositionx+x].material.color.setHex( 0x4caf50 );
                        }
                    }
                    else{
                        cubePaddedDilatedObj[paddedDilateArray.length-1-y+updatedPositiony][updatedPositionx+x].material.color.setHex( 0x3a4f75 );
                    }
                    kernelObj[paddedDilateArray.length-1-y][x].position.x=updatedPositionx+x+n+nk;
                    kernelObj[paddedDilateArray.length-1-y][x].position.y=y-updatedPositiony;
                    try{
                        kernelText[paddedDilateArray.length-1-y][x].position.x=updatedPositionx+x-0.18+n+nk;
                        kernelText[paddedDilateArray.length-1-y][x].position.y=y-0.2-updatedPositiony;
                    }catch(err){
            ...
            updatedPositionx = (updatedPositionx+1)%(paddedDilateArray[0].length-kernelArray[0].length+1);
            updatedPositiony = updatedPositionx==0 ? ((updatedPositiony+1)<(paddedDilateArray.length-kernelArray.length+1) ? (updatedPositiony+1): 0): updatedPositiony;
        }
            ...
            for(y=paddedDilateArray.length-1;y>=paddedDilateArray.length-1-mk+1;y--){
                for(x=0;x<nk;x++){
                    if(kernelArray[paddedDilateArray.length-1-y][x]=='1'){
                        var pixelValue = parseInt(paddedDilateArray[paddedDilateArray.length-1-y+updatedPositiony][updatedPositionx+x]);
                        cubePaddedDilatedObj[paddedDilateArray.length-1-y+updatedPositiony][updatedPositionx+x].material.color.setHex( 0x4caf50 );
            ...
            drawText(updatedPositionx-0.18+2*n+nk+Math.floor(kernelArray.length),m+Math.floor(kernelArray.length/2)-1-updatedPositiony-0.2,0.1,minValue,false,false);
            updatedPositionx = (updatedPositionx+1)%(paddedDilateArray[0].length-kernelArray[0].length+1);
            updatedPositiony = updatedPositionx==0 ? ((updatedPositiony+1)<(paddedDilateArray.length-kernelArray.length+1) ? (updatedPositiony+1): 0): updatedPositiony;
        }
        ...
        if(imageType=='Binary'){
                ...
                for(x=0;x<nk;x++){
                    if(kernelArray[paddedImageArray.length-1-y][x]=='1'){
                        if(paddedImageArray[paddedImageArray.length-1-y+updatedPositiony][updatedPositionx+x]=='0')
                            ...
                            dilatedImageArray[updatedPositiony][updatedPositionx] = '1';
            ...
            if(isUpdatedWithOne==false){
                ...
                dilatedImageArray[updatedPositiony][updatedPositionx] = '0';
            }
            ...
            var maxValue = 0;
            console.log('grayscale dilation processing');
            colorTheCubeToDefault(paddedImageArray, cubePaddedInputObj);
            for(y=paddedImageArray.length-1;y>=paddedImageArray.length-1-mk+1;y--){
                for(x=0;x<nk;x++){
                    if(kernelArray[paddedImageArray.length-1-y][x]=='1'){
            ...
            dilatedImageArray[updatedPositiony][updatedPositionx] = maxValue;
            drawText(updatedPositionx-0.18+n+nk+Math.floor(kernelArray.length/2),m+Math.floor(kernelArray.length/2)-1-updatedPositiony-0.2,0.1,maxValue,false,false);
            updatedPositionx = (updatedPositionx+1)%(paddedImageArray[0].length-kernelArray[0].length+1);
            updatedPositiony = updatedPositionx==0 ? ((updatedPositiony+1)<(paddedImageArray.length-kernelArray.length+1) ? (updatedPositiony+1): 0): updatedPositiony;
        }
        if(updatedPositionx==0 && updatedPositiony==0){
            playDilation = false;
            console.log(paddedDilateArray,dilatedImageArray);
            getPaddedImage(paddedDilateArray,dilatedImageArray,'erosion');
            drawPaddedImage(paddedDilateArray,n+nk,0,cubePaddedDilatedObj)
            drawImage(n+Math.floor(nk/2)+1,0);
            colorTheCubeToDefault(paddedImageArray, cubePaddedInputObj);
            drawText(updatedPositionx-0.18+n+nk,-1-0.2,0.1,'Dilated Image',false,false);
...
var dilatedImageArray = new Array(m).fill(0).map(() => new Array(n).fill(0));

var paddedImageArray = new Array(m+2*Math.floor(mk/2)).fill(0).map(() => new Array(n+2*Math.floor(nk/2)).fill(0));
var paddedDilateArray = new Array(m+2*Math.floor(mk/2)).fill(0).map(() => new Array(n+2*Math.floor(nk/2)).fill(0));

getPaddedImage(paddedImageArray,imageArray,'dilation')
